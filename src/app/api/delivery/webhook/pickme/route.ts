  ...
  try {
    ...
    switch (event) {
      ...
      case 'order.canceled':
        ...
        break
      default:
        console.log(`Unhandled PickMe event: ${event}`)
    }

    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('PickMe webhook error:', error)
    return NextResponse.json(
...
async function handleOrderCreated(orderData: any, storeSettings: any) {
  // Map PickMe order to local order format
  // TODO: Implement actual order mapping based on PickMe API schema
    ...
    if (product) {
      await prisma.order.create({
        data: {
          ...
          orderSource: OrderSource.PICKME,
          deliveryOrderId: orderData.id,
          deliveryPlatform: DeliveryPlatform.PICKME,
